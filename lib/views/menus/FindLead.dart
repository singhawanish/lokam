...
class FindLeadPage extends StatefulWidget {
  final String period;
  final String type;
  final VoidCallback callback;

  FindLeadPage({Key key, this.period, this.type, this.callback})
      : super(key: key);

  @override
  _FindLeadPageState createState() => _FindLeadPageState();
}

class _FindLeadPageState extends State<FindLeadPage>
    with TickerProviderStateMixin {
  AnimationController animationController;
  List<LeadsListData> leadsList = [];
  List<LeadsListData> searchLeadsList = [];
  ...
  void getLeads() async {
    ...
    if (!internet) {
      ...
      var response = await http.get(
        Uri.parse(BASE_URI +
            (_DSARole ? '/DSA/getCapturedLeads' : '/HMOFMO/getCapturedLeads')),
        headers: {
      ...
      if (response.statusCode == 200) {
        setState(() {
          List<LeadsListData> std = [];
          Map<String, dynamic> map = jsonDecode(response.body);
          List<dynamic> data = map['data'];
          data.forEach((element) {
            std.add(LeadsListData(
                    ...
                    Expanded(
                      child: NestedScrollView(
                        ...
                        body: Container(
                          ...
                          child: leadsList.length != 0
                              ? searchLeadsList.length != 0 ||
                                  ...
                                  ? RefreshIndicator(
                                      ...
                                      child: ListView.builder(
                                        shrinkWrap: true,
                                        primary: false,
                                        itemCount: searchLeadsList.length,
                                        ...
                                        itemBuilder:
                                          ...
                                          return LeadsListView(
                                              callback: getLeads,
                                              leadData: searchLeadsList[index],
                                              animation: animation,
                                              animationController:
                                                  animationController,
                                              parentContext:
  ...
  onSearchTextChanged(String text) async {
    ...
    leadsList.forEach((userDetail) {
      if (userDetail.id.toUpperCase().contains(text.toUpperCase()) ||
        ...
        searchLeadsList.add(userDetail);
      }
    });
