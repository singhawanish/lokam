...
class ActivitiesPage extends StatefulWidget {
  ...
  ActivitiesPage({Key key, this.period, this.id, this.callback})
      : super(key: key);
...
class _ActivitiesPageState extends State<ActivitiesPage>
  ...
  List<ActivityListData> openActivitiesList = [];
  List<ActivityListData> closeActivitiesList = [];
  List<ActivityListData> notesList = [];
  ...
  void getActivities() async {
    ...
    if (!internet) {
      ...
      var response = await http.post(
          Uri.parse(BASE_URI +
              (_DSARole
                  ? '/DSA/getLeadOpenActivities'
                  : '/HMOFMO/getLeadOpenActivities')),
          headers: {
      ...
      if (response.statusCode == 200) {
        setState(() {
          List<ActivityListData> std = [];
          Map<String, dynamic> map = jsonDecode(response.body);
          List<dynamic> data = map['data'];
          data.forEach((element) {
            std.add(ActivityListData(
      ...
      response = await http.post(
          Uri.parse(BASE_URI +
              (_DSARole
                  ? '/DSA/getLeadCloseActivities'
                  : '/HMOFMO/getLeadCloseActivities')),
          headers: {
            "Accept": "application/json",
            "Authorization": jwt,
            'Content-type': 'application/json'
      ...
      response = await http.post(
          Uri.parse(BASE_URI +
              (_DSARole ? '/DSA/getLeadNotes' : '/HMOFMO/getLeadNotes')),
          headers: {
            "Accept": "application/json",
            "Authorization": jwt,
            'Content-type': 'application/json'
