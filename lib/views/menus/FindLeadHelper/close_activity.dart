...
class CloseActivityPage extends StatefulWidget {
  const CloseActivityPage(
      {Key key, this.id, this.lat, this.long, this.callback})
      : super(key: key);
  ...
  @override
  _CloseActivityState createState() => _CloseActivityState();
}

class _CloseActivityState extends State<CloseActivityPage> {
                  ...
                  child: Form(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '*Present Location will be Captured.',
                        ),
                        SizedBox(
                          height: 30.0,
                        ),
    ...
    if (_isFormValidated()) {
      ...
      if (!internet) {
        ...
        var response = await http.post(
            Uri.parse(BASE_URI +
                (_DSARole
                    ? '/DSA/closeLeadActivity'
                    : '/HMOFMO/closeLeadActivity')),
            headers: {
                ...
                .showSnackBar(
                  SnackBar(
                    content: WillPopScope(
                      ...
                      child: Text('Activity Closure Failed'),
                    ),
                  ),
                )
                .closed
