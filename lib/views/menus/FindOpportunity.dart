...
import 'FindOpportunityHelper/opportunities_list_view.dart';

class FindOpportunityPage extends StatefulWidget {
  final String period;
  final String type;
  final VoidCallback callback;

  FindOpportunityPage({Key key, this.period, this.type, this.callback})
      : super(key: key);

  @override
  _FindOpportunityPageState createState() => _FindOpportunityPageState();
}

class _FindOpportunityPageState extends State<FindOpportunityPage>
    with TickerProviderStateMixin {
  AnimationController animationController;
  List<OpportunitiesListData> opportunitiesList = [];
  List<OpportunitiesListData> searchOpportunitiesList = [];
  ...
  void getOpportunities() async {
    ...
    if (!internet) {
      ...
      var response = await http.get(
        Uri.parse(BASE_URI +
            (_DSARole
                ? '/DSA/getCapturedOpportunities'
                : '/HMOFMO/getCapturedOpportunitiesCampaign')),
        //   Uri.parse(BASE_URI +
        //   (_DSARole
        //   ? '/DSA/getCapturedOpportunities'
        //   : '/HMOFMO/getCapturedOpportunities')),
        headers: {
      ...
      if (response.statusCode == 200) {
        setState(() {
          List<OpportunitiesListData> std = [];
          Map<String, dynamic> map = jsonDecode(response.body);
          List<dynamic> data = map['data'];
          data.forEach((element) {
            std.add(OpportunitiesListData(
              ...
              convertedToAccount: element['convertedToAccount'] == null
                  ? ''
                  : element['convertedToAccount'] as String,
              ...
              monthlyRequirement: element['monthlyRequirement'] == null
                  ? ''
                  : element['monthlyRequirement'] as String,
              presentlySourcingFrom: element['presentlySourcingFrom'] == null
                  ? ''
                  : element['presentlySourcingFrom'] as String,
              remarkForIOC: element['remarkForIOC'] == null
                  ? 'None'
                  : element['remarkForIOC'] as String,
              productCategory: element['productCategory'] == null
                  ? ''
                  : element['productCategory'] as String,
              leadAssignedTo: element['leadAssignedTo'] == null
                  ? ''
                  : element['leadAssignedTo'] as String,
              leadFunction: element['leadFunction'] == null
                  ? ''
                  : element['leadFunction'] as String,
              selfAssignedYN: element['selfAssigned'] == null
                  ? ''
                  : element['selfAssigned'] as String,
              whetherOMCCustomer: element['whetherOMCCustomer'] == null
                  ? ''
                  : element['whetherOMCCustomer'] as String,
            ));
          });

          opportunitiesList = std;
                    ...
                    Expanded(
                      child: NestedScrollView(
                        ...
                        body: Container(
                          ...
                          child: opportunitiesList.length != 0
                              ? searchOpportunitiesList.length != 0 ||
                                  ...
                                  ? RefreshIndicator(
                                      ...
                                      child: ListView.builder(
                                        ...
                                        primary: false,
                                        itemCount:
                                            searchOpportunitiesList.length,
                                        ...
                                        itemBuilder:
                                          ...
                                          return OpportunitiesListView(
                                              callback: getOpportunities,
                                              opportunityData:
                                                  searchOpportunitiesList[
                                                      index],
                                              animation: animation,
                                              animationController:
                                                  animationController,
  ...
  onSearchTextChanged(String text) async {
    ...
    opportunitiesList.forEach((userDetail) {
      if (userDetail.id.toUpperCase().contains(text.toUpperCase()) ||
          ...
          userDetail.location.toUpperCase().contains(text.toUpperCase()) ||
          userDetail.city.toUpperCase().contains(text.toUpperCase()) ||
          userDetail.industry.toUpperCase().contains(text.toUpperCase()) ||
          userDetail.leadAssignedTo
              .toUpperCase()
              .contains(text.toUpperCase())) {
        searchOpportunitiesList.add(userDetail);
      }
    });
