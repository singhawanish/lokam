...
import 'EscalationsHelper/escalations_list_view.dart';
...
class EscalationsPage extends StatefulWidget {
  ...
  EscalationsPage({Key key, this.period, this.type, this.callback})
      : super(key: key);

  @override
  _EscalationsPageState createState() => _EscalationsPageState();
}

class _EscalationsPageState extends State<EscalationsPage>
    with TickerProviderStateMixin {
  AnimationController animationController;
  List<EscalationsListData> escalationsList = [];
  List<EscalationsListData> searchEscalationsList = [];
  final ScrollController _scrollController = ScrollController();
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();

  TextEditingController searchController = new TextEditingController();
  bool escalationsListPresent = true;
  ...
  void getOpenActivities() async {
    ...
    if (!internet) {
      ...
      var response = await http.get(
        Uri.parse(BASE_URI +
            (_DSARole ? '/DSA/getEscalations' : '/HMOFMO/getEscalations')),
        headers: {
      ...
      if (response.statusCode == 200) {
        setState(() {
          List<EscalationsListData> std = [];
          Map<String, dynamic> map = jsonDecode(response.body);
          List<dynamic> data = map['data'];
          data.forEach((element) {
            std.add(EscalationsListData(
                    ...
                    Expanded(
                      child: NestedScrollView(
                        ...
                        body: Container(
                          ...
                          child: escalationsList.length != 0
                              ? searchEscalationsList.length != 0 ||
                                  ...
                                  ? RefreshIndicator(
                                      ...
                                      child: ListView.builder(
                                        shrinkWrap: true,
                                        primary: false,
                                        itemCount: searchEscalationsList.length,
                                        ...
                                        itemBuilder:
                                          ...
                                          return EscalationsListView(
                                              callback: getOpenActivities,
                                              escalationsData:
                                                  searchEscalationsList[index],
                                              animation: animation,
                                              animationController:
                                                  animationController,
                                              parentContext:
                                  ...
                                  : RefreshIndicator(
                                      ...
                                      child: ListView.builder(
                                        shrinkWrap: true,
                                        primary: false,
                                        itemCount: escalationsList.length,
                                        padding: const EdgeInsets.only(top: 8),
                                        scrollDirection: Axis.vertical,
                                        itemBuilder:
                                            (BuildContext context, int index) {
  ...
  onSearchTextChanged(String text) async {
    ...
    escalationsList.forEach((userDetail) {
      if (userDetail.description.toUpperCase().contains(text.toUpperCase()) ||
        ...
        searchEscalationsList.add(userDetail);
      }
    });
